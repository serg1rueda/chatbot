...
temas_disponibles = [
    "riesgos", "aspectos", "impacto",
    "procedimientos", "comites",
    "emergencias", "responsabilidades"
]
...
def chat():
    try:
        ...
        if pregunta in temas_disponibles:
            ...
            # 👉 Mostrar de una vez info + primera pregunta
            respuestas = {}
            idx = 0
        ...
        if estado == "confirmar_responder":
            if pregunta in ("si", "sí", "s"):
                ...
                if tipo == "info":
                    ...
                    pregunta_text = lines[0] if len(lines) > 1 else contenido
                    opciones = lines[1:] if len(lines) > 1 else (contenido.split(";")[1:] if ";" in contenido else [])
        ...
        if tema_actual and estado == "en_curso":
            ...
            # Aquí sigue la misma lógica de validar respuestas correctas/incorrectas
            # (se mantiene igual a tu versión, solo corregimos arriba la parte de selección de tema)
